...
class MapComponent extends Component {
  ...
  componentDidMount() {
    ...
    // add event markers here maybe.

    console.log(this.props.eventsGeojson);

    this.props.eventsGeojson.features.forEach(function(marker) {
    // create a DOM element for the marker
    var el = document.createElement('div');
    el.className = 'eventmarker';
    el.id = marker.properties.id;
    el.style.backgroundImage = 'url(lovebanner.svg)';
    el.style.width =  '40px';
    el.style.height = '30px';

  // el.addEventListener('click', function() {
//        window.alert(marker.properties.name);
        //console.log('clicked on event marker, should do something with side info thing')
  //  });

    var poptext = "<h3>" +  marker.properties.name + '</h3> <p>' + marker.properties.date+ '</p> <a class="eventLink" target="_blank" href="' + marker.properties.website + '">Event website</a></p>'
    // create the popup
  ...
  componentDidUpdate() {
    ...
    if (this.props.needMapRepan) {
      ...
      if (this.props.mode === "events"){
        console.log('need to repan for event maybe also open the popup');

      //  this.moveTo(this.props.eventLocation);
      let location = this.props.eventLocation;
        console.log('thismap', this.map)
      //  this.map.fire('click', {lnglat: this.props.eventLocation.geometry.coordinates})
      this.map.easeTo({
  ...
  onLoad() {
    ...
    const geometry = {type: 'Point', coordinates: [data.coords.longitude, data.coords.latitude]};
      ...
      if (this.props.moveOnLoad) console.log('woula moved to user loation')
    ...
    if (this.props.userLocation) {
      ...
      //this.moveTo(this.props.userLocation, 13);
        console.log('cancelled the auto move to userlocation');
    }
...
const mapStateToProps = (state) => {
  return {
    ...
    center:[ state.app.mapCoords[0], state.app.mapCoords[1]],
    ...
    eventsGeojson: state.app.eventsGeojson,
